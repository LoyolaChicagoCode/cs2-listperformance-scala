package cs271.lab.list

import org.scalatest.{BeforeAndAfterEach, FunSuite}

class TestList extends FunSuite with BeforeAndAfterEach{
  var list: java.util.ArrayList[Int] = _

  override def beforeEach(){
    list = new java.util.ArrayList[Int]()
    // TODO also try with a LinkedList - does it make any difference?
  }

  override def afterEach(){
    list = null
  }

  test("testSizeEmpty"){
    assert(list.isEmpty() == true)
    assert(list.size() == 0)
    try {
      list.get(0)
      fail("there should not be any items in the list");
    } catch {
      case e: Exception => e
    }
  }

  test("testSizeNonEmpty"){
    // TODO fix the expected values in the assertions below
    list.add(77)
    assert(list.isEmpty() == true)
    assert(list.size() == 0)
    assert(list.get(0).intValue() == 0)
  }

  test("testContains") {
    // TODO write assertions using
    // list.contains(77)
    // that hold before and after adding 77 to the list
    fail("Not yet implemented") // remove this line when done
  }

  test("testAddMultiple") {
    list.add(77)
    list.add(77)
    list.add(77)

    // TODO fix the expected values in the assertions below
    assert(list.size() == 0)
    assert(list.indexOf(77) == 0)
    assert(list.get(1).intValue() == 0)
    assert(list.lastIndexOf(77) == 0)
  }

  test("testAddMultiple2") {
    list.add(33)
    list.add(77)
    list.add(44)
    list.add(77)
    list.add(55)
    list.add(77)
    list.add(66)

    // TODO fix the expected values in the assertions below
    assert(list.size() == 0)
    assert(list.indexOf(77) == 0)
    assert(list.lastIndexOf(77) == 0)
    assert(list.get(2).intValue() == 0)
    assert(list.get(3).intValue() == 0)
    assert(List(33, 77, 44) == list)
  }

  test("testRemoveObject") {
    list.add(3)
    list.add(77)
    list.add(4)
    list.add(77)
    list.add(5)
    list.add(77)
    list.add(6)
    list.remove(5)  // TODO what does this method do?

    // TODO fix the expected values in the assertions below
    assert(list.size() == 0)
    assert(list.indexOf(77) == 0)
    assert(list.lastIndexOf(77) == 0)
    assert(list.get(2).intValue() == 0)
    assert(list.get(3).intValue() == 0)
    list.remove(Integer.valueOf(5)) // TODO what does this one do?
    assert(list.size() == 0)
    assert(list.indexOf(77) == 0)
    assert(list.lastIndexOf(77) == 0)
    assert(list.get(2).intValue() == 0)
    assert(list.get(3).intValue() == 0)
  }

  test("testContainsAll") {
    list.add(33)
    list.add(77)
    list.add(44)
    list.add(77)
    list.add(55)
    list.add(77)
    list.add(66)

    // TODO using containsAll and Arrays.asList (see above),
    // 1) assert that list contains all five different numbers added
    // 2) assert that list does not contain all of 11, 22, and 33
    fail("Not yet implemented"); // remove this line when done
  }

  test("testAddAll") {
    // TODO in a single statement using addAll and Arrays.asList,
    // add items to the list to make the following assertions pass
    // (without touching the assertions themselves)
    assert(list.size() == 7)
    assert(list.get(0).intValue() == 33)
    assert(list.get(1).intValue() == 77)
    assert(list.get(2).intValue() == 44)
    assert(list.get(3).intValue() == 77)
    assert(list.get(4).intValue() == 55)
    assert(list.get(5).intValue() == 77)
    assert(list.get(6).intValue() == 66)
  }

  test("testRemoveAll") {
    list.add(33)
    list.add(77)
    list.add(44)
    list.add(77)
    list.add(55)
    list.add(77)
    list.add(66)

    // TODO in a single statement using removeAll and Arrays.asList,
    // remove items from the list to make the following assertions pass
    // (without touching the assertions themselves)
    assert(list.size() == 3)
    assert(List(77, 77, 77) == list)
  }

  test("testRetainAll") {
    list.add(33)
    list.add(77)
    list.add(44)
    list.add(77)
    list.add(55)
    list.add(77)
    list.add(66)

    // TODO in a single statement using retainAll and Arrays.asList,
    // remove items from the list to make the following assertions pass
    // (without touching the assertions themselves)
    assert(list.size() == 3)
    assert(List(77, 77, 77) == list)
  }

  test("testSet") {
    list.add(33)
    list.add(77)
    list.add(44)
    list.add(77)
    list.add(55)
    list.add(77)
    list.add(66)

    // TODO use the set method to change specific elements in the list
    // such that the following assertions pass
    // (without touching the assertions themselves)
    assert(list.size() == 7)
    assert(list.get(0).intValue() == 33)
    assert(list.get(1).intValue() == 99)
    assert(list.get(2).intValue() == 44)
    assert(list.get(3).intValue() == 99)
    assert(list.get(4).intValue() == 55)
    assert(list.get(5).intValue() == 99)
    assert(list.get(6).intValue() == 66)
  }

  test("testSubList") {
    list.add(33)
    list.add(77)
    list.add(44)
    list.add(77)
    list.add(55)
    list.add(77)
    list.add(66)

    // TODO fix the arguments in the subList method so that the assertion
    // passes
    assert(List(44, 77, 55) == list.subList(0, 0))
  }

}